...
var Filesystem = class {
  ...
  async insertFileAsync(p, streamGenerator, file, node, transformed) {
    const tmpdir2 = await wrappedFs.mkdtemp(path2.join(os.tmpdir(), "asar-"));
    const tmpfile = path2.join(tmpdir2, path2.basename(p));
...
function isUnpackedDir(dirPath, pattern, unpackDirs) {
  if (dirPath.startsWith(pattern) || minimatch(dirPath, pattern)) {
    ...
    for (const unpackDir of unpackDirs) {
      if (dirPath.startsWith(unpackDir) && !path5.relative(unpackDir, dirPath).startsWith("..")) {
        return true;
      }
    }
    return false;
...
async function createPackageFromFiles(src, dest, filenames, metadata = {}, options = {}) {
  ...
  for (const filename of filenamesSorted) {
    ...
    switch (file.type) {
      case "directory":
        shouldUnpack = shouldUnpackPath(filename, path5.relative(src, filename), void 0, options.unpackDir);
        filesystem.insertDirectory(filename, shouldUnpack);
        break;
      case "file":
        shouldUnpack = shouldUnpackPath(filename, path5.relative(src, path5.dirname(filename)), options.unpack, options.unpackDir);
        ...
        break;
      case "link":
        shouldUnpack = shouldUnpackPath(filename, path5.relative(src, filename), options.unpack, options.unpackDir);
  ...
  for (const fullPath of filenames) {
    ...
    if ("files" in file) {
      ...
      try {
        let content;
        if (file.unpacked) {
          content = wrappedFs.readFileSync(path5.join(`${filesystem.getRootPath()}.unpacked`, filename));
        } else if (file.size <= 0) {
...
var workPath = await mkdtemp(join(tmpdir(), "atoapi-codex-ui-"));
try {

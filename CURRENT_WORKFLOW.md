...
## AUTHORITATIVE ACTIVE STATE ‚Äî 2026-08-08 (supersedes the 2026-07-30 release block below)
...
- The accepted frozen base is **v1.4.33**, packaged at
  ...
  The source commit is `85b56a6` (`feat: ÂèëÂ∏É v1.4.33 ÂëΩ‰∏≠ÊàêÁÜü„ÄÅÊµãÊ¥ª‰∏é Agent ÈöîÁ¶ª‰øÆÂ§ç`).
...
- **50‰∏áÁ∫ß control is invalid, not a binary result:** v1.4.33 versus the same
...
### Latest governed return ‚Äî 2026-08-11 (current sub5 scope)
...
- **2026-08-11 same-binary capacity control:** a one-pair v1.4.33-versus-v1.4.33
  ...
  `150K‚Äì200K` context. It is diagnostic-only: with one crossover pair the
...
- **Decision:** keep v1.4.33 as the champion. Do not fold either artifact into
  ...
  reach the real `150K‚Äì200K` token gate and the selected upstream completes
  both arms; protocol/schema probes remain verifier-only and must not alter
  production cache behavior.

### Current execution handoff ‚Äî 2026-08-11 (actual `agent-codex-20` cohort)
...
- **2026-08-11 verifier scope-gate repair:** the live gate now selects the
  ...
  `agent-codex-20 / gpt-5.6-terra / realm a4d4fff6998b‚Ä¶`, with multi-Key pin
...
- **2026-08-11 immediate scope refresh:** immediately before the planned
  ...
  hand-selected Key realm, `286e5e04583f‚Ä¶` (multi-Key pin enabled) at
  `2026-08-11T15:21:35.1008160Z`. This is a new cohort; do not mix it with
  the earlier `a4d4fff6998b‚Ä¶` preflight or use either preflight as a cache
...
- **2026-08-11 new-realm same-binary control invalid:** the first pair of the
  `286e5e04583f‚Ä¶` 75K dynamic text-tail shared crossover used the same
  v1.4.33 SHA (`bca193‚Ä¶`) for both slots and bound the same Provider/model/
...
- **Current hand-selected scope:** `agent-codex-20 / gpt-5.6-terra / realm
  a4d4fff6998b‚Ä¶`. Do not combine this cohort with the earlier sub5 /
  `7235‚Ä¶` artifacts. The current realm has 110 complete terminal flows with
...
- **2026-08-11 BIZD Sol scope-drift control:** a fresh interactive scope read
  resolved `agent-codex-bizd / gpt-5.6-sol / realm 23850acc‚Ä¶` with no
...
- **2026-08-11 BIZD Terra stable full-replay self-control:** two consecutive
  ...
  c416f6c1‚Ä¶` with the current multi-Key selection pinned. A two-pair 75K
...
- The current hand-selected Codex scope was refreshed immediately before the
  ...
  7235fe2d0a2d‚Ä¶`, observed at `2026-08-11T05:21:07.7394469Z`. The resolver
...
- The current manually selected Codex upstream later moved to a separate
  cohort, `agent-codex-20 / gpt-5.6-terra / realm a4d4fff6998b‚Ä¶`. The
  interactive resolver mapped that realm to the currently healthy pinned Key
  at each attempted run; do not mix this cohort with the earlier `7235‚Ä¶`
  sub5 evidence.
...
- First live isolated pilot (2026-08-08) used the packaged v1.4.33 EXE as
  ...
  `agent-codex-provider-4 / gpt-5.6-terra / realm 4611‚Ä¶9673`,
...
- Re-saving the `ËøêÊ≤≥ / agent-codex-provider-4` record updated its on-disk
...
- A desktop-principal 1,000,000-character `full-replay` ladder seed then
  ...
  `agent-codex-bizd / gpt-5.6-terra / Key 2 / realm 7bf7‚Ä¶9cab7` scope. Both
...
- During the next return, Codex injection changed the live scope to
   ...
   tokens and matched realm `7bcb‚Ä¶4ab6`; however one arm had `274,432`
...
- A separate Codex injection-stability repair is now present in the uncommitted
 ...
 - A guarded 1.5M candidate run detected a live scope drift before any
   ...
   `4574f5‚Ä¶5b407` instead of the pinned `7bcb‚Ä¶4ab6`. The verifier failed
 ...
 - After rebinding the current provider2 Key, the actual scope
   `4574f5‚Ä¶5b407` completed a 1.5M candidate comparison with both arms at
 ...
 - A smaller dual-lane stability gate on the same `4574f5‚Ä¶5b407` realm completed
...
- A 1.5M diagnostic with distinct per-arm prompt-cache keys made seed cache
 ...
 - The first valid 1.5M candidate comparison on the current `4574f5‚Ä¶5b407`
...
- Post-clean live comparison confirms the user's perceived v1.4.34 hit drop is
  ...
  metrics, `agent-codex-provider-2 / realm 4574f5c2‚Ä¶ / gpt-5.6-terra` reached
  `3,801,638` input tokens and `3,717,120` cached (`97.7768%`) across 16
  successful rows, with `cache_avoidable_gap_tokens=0`. The mixed
  `agent-codex-bizd / realm 7bf7f91b‚Ä¶` slice had `8,350,127` input tokens and
...
- Historical v1.4.33 release cohorts show the accepted positive line is
  ...
  `agent-codex-provider-6 / realm bc8fca9d‚Ä¶` at `36,865,244` input and
...
- A read-only route audit after the v1.4.34 install confirms the enabled Codex
  ...
  `provider-2 / realm 4574f5c2‚Ä¶ / gpt-5.6-terra / Responses`.
...
- A fresh read-only snapshot at `2026-08-08T13:57:57Z` confirms the live
  ...
  gpt-5.6-terra / realm 4574f5c2‚Ä¶` slice, all 200 retained recent rows
...
- A follow-up live final-scope audit of the same 200 `provider-2 / terra /
  realm4574‚Ä¶` rows found 159 rows with a positive sent-vs-settled residual
  ...
  Only five rows fell inside the candidate's newly widened `4,096‚Äì16,384`
...
- The new natural-dense dynamic verifier path was exercised against the current
  ...
  required peak range of 450K‚Äì500K input tokens. The v1.4.33 champion completed
...
- The prior provider-2 450K‚Äì500K natural-dense dynamic ladder remains
...
- This is not a context-capacity ceiling or cache regression. It is a
  ...
  invalid and the 256k/384k/450‚Äì500k ladder is paused. Do not retry the failed
...
- v1.4.37 is an additive UI/package checkpoint built on v1.4.36. On an
  ...
  `N ‰∏™ Key` chip immediately to the left of the existing balance chip.
...
450K‚Äì500K natural-dense shape again. Items 3, 3a, and 5a below are retained
...
2. Before deploying the source-built injection repair, retain the existing
 ...
 3. **Historical/superseded:** the Codex injection scope was `agent-codex-bizd / gpt-5.6-terra`
   with explicit `Key 2` and observed realm `7bf7‚Ä¶9cab7`; do not reuse the
   retired `agent-codex-provider-4 / realm 4611‚Ä¶` scope. The verifier wrapper
 ...
 3a. **Historical/superseded:** the scope override from the latest 20:03 snapshot used
     ...
     `7bcb‚Ä¶4ab6`. The preceding bizd/Key-2 observations remain historical
 ...
 5a. **Historical/superseded:** `scripts/run-release-champion-interactive.ps1` previously defaulted to the
    observed current scope (`agent-codex-provider-2 / realm 4574f5c2‚Ä¶`),
...
The older 2026-07-29 draft below is retained as historical evidence; this release-state block overrides its outdated ‚Äúun-packaged‚Äù wording.

## AUTHORITATIVE ACTIVE LINE ‚Äî 2026-07-29 (read this before older history)
...
### Unpackaged full-replay safety follow-up ‚Äî 2026-07-30
...
## 2026-08-13 ø…º˚’˝Œƒ ◊◊÷”Î’Ê µ—”≥Ÿ’Ô∂œ

- œ÷œÛ£∫«·¡ø≤‚ªÓµƒ°∞ ◊∏ˆ∑µªÿ ¬º˛°±≤ªµ»”⁄’Ê µ«Î«Û ◊◊÷£ª≤‚ªÓ π”√ `stream=true`°¢`max_output_tokens=1`£¨ ’µΩπ‹¿Ì/¥¥Ω® ¬º˛º¥ø…Ω· ¯£¨≤ªƒ‹¥˙±Ìƒ£–Õ’˝Œƒ ◊◊÷°£
- ’Ô∂œ£∫’Ê µ«Î«Ûµƒ±æµÿ◊º±∏Õ®≥£‘º 2®C40ms£¨prefix wait ”–‘º 500ms ”≤…œœﬁ£¨gzip ‘º 1®C6ms£¨œ¬”Œ backpressure Œ™ 0£ª÷˜“™—”≥Ÿ¿¥◊‘œµÕ≥¥˙¿Ì/…œ”Œ headers ”Î ◊∏ˆ SSE  ‰≥ˆ£¨¥˙±Ì–‘«Î«Ûµƒ `upstream_headers_ms` ø…¥Ô ˝ Æ√ÎµΩ¡Ω∞Ÿ√Î°£
- ¥˙¬Îπ€≤‚£∫∫Û∂À–¬‘ˆ `visible_text_ttft_ms`£¨÷ª‘⁄ Responses/Chat/Anthropic µƒø…º˚Œƒ±æªÚæ‹¥Œƒ±æ ◊∏ˆ delta µΩ¥Ô ±º«¬º£ªreasoning°¢function-call ≤Œ ˝∫Õπ§æﬂ‘™ ˝æ›≤ªª·√∞≥‰’˝Œƒ ◊◊÷°£UI ºÃ–¯œ‘ æƒ£–Õ ◊◊÷£¨≤¢‘⁄œÍ«Èœ‘ æø…º˚’˝Œƒ ◊◊÷∫Õ∂¿¡¢µƒ ◊∏ˆ…œ”Œ ¬º˛°£
- —È÷§£∫`node scripts/test-graphite-latency-labels.mjs`°¢`node scripts/test-provider-health-probe-ui.mjs`°¢`node scripts/test-graphite-key-pool-ui.mjs`°¢`npm.cmd run -s build`°¢`cargo check --manifest-path src-tauri/Cargo.toml` æ˘Õ®π˝£ªΩˆ¥Ê‘⁄º»”– dead-code warning°£
- A/B Ω·¬€£∫`output/latency-v1433-v1437-realshape-20260813-150904.json` Œﬁ”––ß∞Ê±æΩ·¬€£ªπ⁄æ¸ ◊∏ˆ 600K seed ‘⁄…œ”Œ `capacity` ¥ÌŒÛ£¨∫Ú—°Œ¥–Œ≥…«Î«Û£¨≤ªƒ‹æ›¥ÀΩ˙º∂ªÚÃÊªª live°£¥À«∞Œ»∂®Õ¨∂˛Ω¯÷∆—˘±æœ‘ æ±æµÿ p95 ‘º≤Ó 2ms°¢…œ”Œ TTFT ≤Ó‘º 2452ms£¨Œ¥÷ß≥÷–ﬁ∏ƒª∫¥Ê/prefix wait/gzip/live ¬∑”…°£
- œ¬“ª≤Ω£∫‘⁄◊¿√Ê”√ªß `desktop-jrsahpg\\msj` µƒ ÷—°…œ”Œ≈‰÷√ø…Ω‚√‹«“ µº ø…”√ ±£¨À¢–¬ hand-selected scope£¨∞¥∂‘≥∆∂ØÃ¨…œœ¬Œƒ÷ÿ–¬≈‹ paced `150K®C230K` Õ¨∂˛Ω¯÷∆øÿ÷∆£¨‘Ÿ≤‚∫Ú—°£ª±£≥÷ `v1.4.33` Œ™π⁄æ¸ª˘œﬂ£¨Œ¥–Œ≥…Œﬁ¥ÌŒÛ°¢Œﬁ∂ÓÕ‚¿‰∆Ù∂Ø«“√¸÷–≤ªµÕ”⁄ª˘œﬂµƒ’˝”≈ªØ«∞≤ªΩ˙º∂°£

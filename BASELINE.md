...
Current packaged candidate: `v0.1.35-atoapi-compact-diagnostics-cache-error-isolation-20260628`

Release folder:

`releases/v0.1.35-atoapi-compact-diagnostics-cache-error-isolation-20260628`
...
- v0.1.35 is the current two-line package. Compression line: it keeps v0.1.34 Chat stream aggregation for large old conversations, adds backend-only `upstream_headers_ms`, `upstream_first_chunk_ms`, `aggregate_done_ms`, and `sse_chunks` diagnostics on compact/non-stream aggregation paths, and uses a conservative Chat non-stream fast path only when the transformed Chat body is small enough and tail/tool-output risk is low. Hit-rate line: it isolates upstream/SSE error bodies from prefix learning and local cache writes, so `response.failed`, `response.incomplete`, SSE error events, and 200 error JSON do not poison waterlines or count as normal misses. It remains one upstream request, does not add active prewarm, does not add companion sync calls, and does not restore normal main session-delta.
